...
function sandbox(name) {
  ...
  use) echo "use $2" >> "${home}/fixtures/calls.log"; [ "$2" = "failme@test" ] && exit 1; exit 0;;
...
(async () => {
  ...
  // ── S1: --help
  {
    ...
    check('help exits 0 and documents subcommands', r.status === 0 && r.stdout.includes('codex-add') && r.stdout.includes('codex-use'), r.stdout);
  }
  ...
  // ── S3: claude hot → real switch through fake claude-acct
  {
    ...
    check('claude switch notifies', sb.read('fixtures/notify.log').includes('Claude switched'));
    check('claude switch rendered', r.stdout.includes('switched claude a@test'), r.stdout);
  }
